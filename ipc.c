...
//
//                if (fcntl(pipes[i][j * 2], F_SETFL, O_NONBLOCK) < 0) {
//                    printf("Failed to set non-blocking mode for pipe: error=%s\n", strerror(errno));
//                    exit(1);
//                }
//                if (fcntl(pipes[i][j * 2 + 1], F_SETFL, O_NONBLOCK) < 0) {
//                    printf("Failed to set non-blocking mode for pipe: error=%s\n", strerror(errno));
//                    exit(1);
//                }
//            }
...
//
//    for (i = 0; i <= pipe->total_p; i++) {
//        pipe->write_pipes[i] = pipes[0][i * 2 + 1];
...
//
//    memcpy(&buffer[serialized_size], message->s_payload, header.s_payload_len);
...
//    while ((bytes_written = write(pipe->write_pipes[dst], buffer, serialized_size)) == -1) {
//        if (errno != EAGAIN) {
//            printf("Failed to write to pipe: descriptor=%d error=%s\n", pipe->write_pipes[dst], strerror(errno));
//            return 1;
//        }
//    }
...
//            int bytes_written;
//
//            while ((bytes_written = write(pipe->write_pipes[i], buffer, serialized_size)) == -1) {
//                if (errno != EAGAIN) {
//                    printf("Failed to write to pipe: descriptor=%d error=%s\n", pipe->write_pipes[i], strerror(errno));
//                    return 1;
//                }
//            }
//
...
//    pipe = (Pipe *) self;
//
//    while (read(pipe->read_pipes[from], buffer, sizeof(MessageHeader)) == -1) {
//        if (errno != EAGAIN) {
//            printf("Failed to read from pipe: descriptor=%d error=%s\n", pipe->read_pipes[from], strerror(errno));
//            exit(1);
//        }
//    }
//    deserialize_header(buffer, &msg->s_header);
//
//    while (read(pipe->read_pipes[from], &buffer[sizeof(MessageHeader)], msg->s_header.s_payload_len) == -1) {
//        if (errno != EAGAIN) {
//            printf("Failed to read from pipe: descriptor=%d error=%s\n", pipe->read_pipes[from], strerror(errno));
//            exit(1);
//        }
//    }
//    memcpy(msg->s_payload, &buffer[sizeof(MessageHeader)], msg->s_header.s_payload_len);
//
//    return 0;
//}
//
...
//    char buffer[MAX_MESSAGE_LEN];
//
//    pipe = (SenderPipe *) self;
//    while (1) {
//        int i;
//        for (i = 0; i <= pipe->pipe.total_p; i++) {
//            if (i != pipe->pipe.local_id) {
//                int bytes_read;
//
//                bytes_read = read(pipe->pipe.read_pipes[i], buffer, sizeof(MessageHeader));
//                if (bytes_read > 0) {
//                    if (bytes_read != sizeof(MessageHeader)) {
//                        printf("Failed to read from pipe: descriptor=%d\n", pipe->pipe.read_pipes[i]);
//                        return 1;
//                    }
//                    deserialize_header(buffer, &msg->s_header);
//
//                    if (msg->s_header.s_payload_len) {
//                        while (read(pipe->pipe.read_pipes[i], &buffer[sizeof(MessageHeader)], msg->s_header.s_payload_len) == -1) {
//                            if (errno != EAGAIN) {
//                                printf("Failed to read from pipe: descriptor=%d error=%s\n", pipe->pipe.read_pipes[i], strerror(errno));
//                                return 1;
//                            }
//                        }
//                        memcpy(msg->s_payload, &buffer[sizeof(MessageHeader)], msg->s_header.s_payload_len);
