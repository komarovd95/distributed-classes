...
int enqueue(Queue *queue, const CsRequest *request) {
    ...
    while (i > 1 && timestamp_compare(&queue->requests[i / 2], &queue->requests[i]) > 0) {
        swap(queue, i, i / 2);
        i /= 2;
    }
    queue->length++;
...
int dequeue(Queue *queue, CsRequest *request) {
    ...
    while (i * 2 <= queue->length) {
        j = 2 * i;
        if (j < queue->length && timestamp_compare(&queue->requests[j], &queue->requests[j + 1]) > 0) {
            j++;
        }
        if (timestamp_compare(&queue->requests[i], &queue->requests[j]) <= 0) {
            break;
        }
        swap(queue, i, j);
        i = j;
